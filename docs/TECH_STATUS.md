...
| Debug Logs | Partial | Implemented as raw logs/debug visualizer | `SandboxDebugVisualizer`, projectile systems | Watch console volume | Needs DebugLogConfig |
...
- Debug logs such as `[SHOT]`, `[HIT]`, `[BOUNCE]`, and `[ARMOR]` can spam the Console during active testing.
